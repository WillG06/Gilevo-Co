...
const BigPreview = ({
  ...
  // Paused by default — user must press Play
  const [playing, setPlaying] = useState(false);

  // Only run RAF when section is visible
  ...
  // Reset scroll + pause when frame changes or leaving viewport
  ...
  // RAF — only active when playing AND in view
  useAnimationFrame((_, delta) => {
    if (!trackRef.current || !playing || !isInView) return;
    const max = trackRef.current.scrollHeight - trackRef.current.clientHeight;
    if (max <= 0) return;
    offset.current += (delta / 1000) * 80;
    if (offset.current >= max) {
      offset.current = max;
      setPlaying(false); // auto-pause when bottom reached
      return;
    }
    trackRef.current.scrollTop = offset.current;
  });
  ...
  const getProgress = () => {
    ...
    const max = trackRef.current.scrollHeight - trackRef.current.clientHeight;
    if (max <= 0) return 0;
    return Math.min((offset.current / max) * 100, 100);
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-hairline bg-paper">
        ...
        {/* Pause/Play toggle — only shown once playing has started */}
        {playing && (
          <button
      ...
      >
        {/* CSS transition on opacity instead of framer-motion for perf */}
        <img
          key={`${project.id}-${activeIndex}`}
          src={frame.src}
          alt={`${project.name} — ${frame.label}`}
          className="w-full block select-none animate-fadeIn"
          draggable={false}
          loading="lazy"
        />
        ...
        {!playing && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-brand-blue-deep/20 backdrop-blur-[2px]">
            <button
              onClick={() => setPlaying(true)}
              aria-label="Play scroll preview"
              className="group flex flex-col items-center gap-3"
            >
              <span className="h-16 w-16 rounded-full bg-background/90 backdrop-blur border border-hairline shadow-lg grid place-items-center group-hover:scale-110 group-hover:bg-brand-blue-deep group-hover:text-background text-brand-blue-deep transition-all duration-200">
      ...
      {/* Thin progress bar at the bottom of the preview */}
      {playing && (
...
/* =================== Per-project section — lazy mounts preview =================== */
const ProjectGallery = ({ project, index }: { project: Project; index: number }) => {
  const [activeIndex, setActiveIndex] = useState(0);
  const sectionRef = useRef<HTMLDivElement>(null);

  // Only mount the BigPreview once the section has entered the viewport
  // (reduces initial render cost — especially image decoding)
  const hasEnteredView = useInView(sectionRef, { once: true, amount: 0.05 });

  const prev = () =>
    setActiveIndex((i) => (i - 1 + project.frames.length) % project.frames.length);
  ...
  return (
    ...
    >
      ...
      {hasEnteredView ? (
        ...
        // Skeleton placeholder keeps layout stable before mount
        <div className="rounded-2xl ring-1 ring-hairline bg-paper h-[560px] lg:h-[680px] animate-pulse" />
      )}
    ...
    <div className="relative grid lg:grid-cols-12 gap-10 items-stretch rounded-3xl overflow-hidden border border-hairline bg-gradient-to-br from-paper via-background to-paper p-8 lg:p-12">
      ...
      <div className="relative lg:col-span-5 grid grid-cols-2 gap-3">
        {[
          ...
          >
            <p className="display-sans text-2xl lg:text-3xl tracking-tighter text-brand-blue-deep">
              {k}
            </p>
